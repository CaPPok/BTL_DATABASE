...
export default function DashboardPage() {
  ...
  // State quản lý tháng hiện tại cho Calendar (Mặc định tháng 12/2025 theo data mẫu)
  const [currentDate, setCurrentDate] = useState(new Date(2025, 11, 1)); // Tháng 12 (index 11)

  const userId = "2300001"; 
  ...
  const formatDateVN = (dateString: string) => {
    ...
    // Ví dụ output: "Thứ Sáu, 12 tháng 12, 2025"
  ...
  // --- LOGIC 2: CALENDAR (NÂNG CẤP) ---
  const renderCalendar = () => {
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();

    // Ngày đầu tháng (1/12) rơi vào thứ mấy? (0=CN, 1=T2... 6=T7)
    // Lưu ý: getDay() trả về 0 cho CN. UI của bạn bắt đầu từ T2. Cần map lại.
    ...
    return (
        <div className="grid grid-cols-7 gap-0 text-sm border-t border-l border-gray-200">
            ...
            {/* Render ngày thực tế */}
            {days.map(day => {
                // Kiểm tra xem ngày này có deadline không
